...
This counter-example is a pathalogical case, and it has been shown that there are many algorithms (\alg{FSSS}, \alg{Sparse Sampling}, \alg{UCT}, and others) that are robust to small differences in the transition function, in general.
...
\begin{bfs3thm}
The expected number of sub-$\epsilon$-optimal steps that \alg{BFS3} will take in the BAMDP is no more than $\frac 1 {\delta_l} H B C$, where
\begin{eqnarray}
\epsilon &=&,\\
\delta_l &=&,\\
H &=&.
...
\end{eqnarray}
That is, the transition and reward functions involving the current belief-state $b_t$ use the true dynamics from the BAMDP. The value of every belief-state other than the one that the agent currently resides in is found by using \alg{FSSS}. This differs from the MDP that \alg{BFS3} uses to choose actions only in the first step of the simulation: \alg{BFS3} will use a Monte-Carlo estimate of the transition and reward functions instead of the true functions.

We can bound the difference between $Q_t(b_t,a)$ and $Q_{B_t}(b_t,a)$:
\begin{eqnarray}
|Q_t(b_t,a) - Q_{B_t}(b_t,a)| \leq \epsilon_c,
\end{eqnarray}
where $\epsilon_c$ is \note{$\epsilon_c$ is well-defined and easy to derive, just not right now. it's a function of the number of next-states $C$ and the sparse sampling paper lists it.}
\end{lemma}
